...
class neutralwhite:public QObject, public AlgorithmShell
{
  ...
  protected:
	...
	void windowAdded(Subject & subject, const std::string & signal,
					 const boost::any & value);
	void windowActivated(Subject & subject, const std::string & signal,
						 const boost::any & value);
	void windowRemoved(Subject & subject, const std::string & signal,
					   const boost::any & value);

	void addMouseMode(SpatialDataView * pView);
	void removeMouseMode(SpatialDataView * pView);
	void enableAction();
	bool copyImage2(RasterElement * pRaster, RasterElement * dRaster, int i,
					double max, double correct);
