...
using namespace std;
XERCES_CPP_NAMESPACE_USE static const char *const MouseModeIcon2[] = {
	"16 16 2 1",
	"# c #000000",
	". c #ffffff",
	"................",
...
{
	template < typename T >
		void copywhite2(T * pData, DataAccessor pSrcAcc, int row, int col,
						int rowSize, int colSize, double correct, double max)
	{
...
bool neutralwhite::getRGB(RasterElement * pRaster,
						  RasterDataDescriptor * pDesc, int x, int y,
						  double correct[])
{
	...
	SpatialDataView *pSpatialDataView =
		dynamic_cast <
		SpatialDataView * >(pDesktop->getCurrentWorkspaceWindowView());
...
bool neutralwhite::copyImage2(RasterElement * pRaster, RasterElement * dRaster,
							  int i, double max, double correct)
{
	VERIFY(pRaster != NULL);
...

	{
			 ...
			 ++curCol)
		{
			...
			switchOnEncoding(pDesc->getDataType(), copywhite2,
							 pDestAcc->getColumn(), thirdBandDa, curRow,
							 curCol, pDesc->getRowCount(),
							 pDesc->getColumnCount(), correct, max);
...
mpMouseMode(NULL), mpMouseModeAction3(NULL)
{
	AlgorithmShell::
		setName("Neutral Reference Selection for white correction");
	setCreator("Pratik Anand");
	setVersion("0.1");
	setCopyright("Copyright (C) 2011, Pratik Anand <pratik@pratikanand.com>");
	setDescription
		("White balance correction of an image based in selected neutral reference");
...
neutralwhite::~neutralwhite()
{
	...
	ToolBar *pToolBar =
		dynamic_cast <
		ToolBar * >(pDesktop->getWindow("Photography", TOOLBAR));
	if (pToolBar != NULL)
	{
	...
	pDesktop->detach(SIGNAL_NAME(DesktopServices, WindowAdded),
					 Slot(this, &neutralwhite::windowAdded));
	pDesktop->detach(SIGNAL_NAME(DesktopServices, WindowActivated),
					 Slot(this, &neutralwhite::windowActivated));
	pDesktop->detach(SIGNAL_NAME(DesktopServices, WindowRemoved),
					 Slot(this, &neutralwhite::windowRemoved));

	// Remove the mouse mode from the views
	vector < Window * >windows;
	pDesktop->getWindows(SPATIAL_DATA_WINDOW, windows);

	for (vector < Window * >::iterator iter = windows.begin();
		 iter != windows.end(); ++iter)
	{
		SpatialDataWindow *pWindow =
			static_cast < SpatialDataWindow * >(*iter);
		if (pWindow != NULL)
		{
...
bool neutralwhite::execute(PlugInArgList * pInArgList,
						   PlugInArgList * pOutArgList)
{
	StepResource pStep("mMode", "m_whitecorr",
					   "5EA0CC75-9E0B-4c3d-BA23-6DB7157BBD54");


	// Progress* pProgress =
	// pInArgList->getPlugInArgValue<Progress>(AlgorithmShell::ProgressArg());
	...
	mpMouseModeAction3 =
		new QAction(mouseModeIcon2,
					"Select the neutral reference for white correction", this);
	mpMouseModeAction3->setAutoRepeat(false);
	mpMouseModeAction3->setCheckable(true);
	mpMouseModeAction3->
		setStatusTip("Select the neutral reference for white correction ");
	...
	if (pToolBar == NULL)
	{
		pToolBar =
			dynamic_cast <
			ToolBar * >(pDesktop->createWindow("Photography", TOOLBAR));
	}
	if (pToolBar != NULL)
	{
	...
	// Connections
	pDesktop->attach(SIGNAL_NAME(DesktopServices, WindowAdded),
					 Slot(this, &neutralwhite::windowAdded));
	pDesktop->attach(SIGNAL_NAME(DesktopServices, WindowActivated),
					 Slot(this, &neutralwhite::windowActivated));
	pDesktop->attach(SIGNAL_NAME(DesktopServices, WindowRemoved),
					 Slot(this, &neutralwhite::windowRemoved));
...
bool neutralwhite::serialize(SessionItemSerializer & serializer) const const
{
	if (mpMouseModeAction3 != NULL)
	{
		...
		vector < Window * >windows;
		Service < DesktopServices > ()->getWindows(SPATIAL_DATA_WINDOW,
												   windows);
		for (vector < Window * >::iterator iter = windows.begin();
			 iter != windows.end(); ++iter)
		{
			SpatialDataWindow *pWindow =
				dynamic_cast < SpatialDataWindow * >(*iter);
			if (pWindow != NULL)
			{
				SpatialDataView *pView = pWindow->getSpatialDataView();
				if (pView != NULL)
...
bool neutralwhite::deserialize(SessionItemDeserializer & deserializer)
{
	...
	DOMElement *pRootElement =
		deserializer.deserialize(reader, windowName.toUtf8());
	if (pRootElement != NULL)
	{
		bool batch =
			StringUtilities::fromXmlString < bool >
			(A(pRootElement->getAttribute(X("batch"))));
		if (batch == true)
		{
			setBatch();
		}
	...
	for (DOMNode * pNode = pRootElement->getFirstChild(); pNode != NULL;
		 pNode = pNode->getNextSibling())
	{
		if (XMLString::equals(pNode->getNodeName(), X("view")))
		{
			...
			SpatialDataView *pView =
				...
				SpatialDataView *
				>(pSession->
				  getSessionItem(A(pViewElement->getAttribute(X("id")))));
			if (pView != NULL)
			{
				QWidget *pViewWidget = pView->getWidget();
				if (pViewWidget != NULL)
		...
		if (pEvent->type() == QEvent::MouseButtonPress)
		{
			...
			if (pMouseEvent->button() == Qt::LeftButton)
			{
				...
				if (pSpatialDataView != NULL)
				{
					...
					{
						MouseMode *pMouseMode =
							pSpatialDataView->getCurrentMouseMode();
						if (pMouseMode != NULL)
						{
							string mouseMode = "";
							...
							if (mouseMode == "DisplayPixelCoordinateMode2")
							{
								...
								ptMouse.setY(pViewWidget->height() -
											 pMouseEvent->pos().y());

								LocationType pixelCoord;
								pSpatialDataView->
									translateScreenToWorld(ptMouse.x(),
														   ptMouse.y(),
														   pixelCoord.mX,
														   pixelCoord.mY);
								...
								double dMaxX = 0.0;
								double dMaxY = 0.0;
								pSpatialDataView->getExtents(dMinX, dMinY,
															 dMaxX, dMaxY);

								if ((pixelCoord.mX >= dMinX)
									&& (pixelCoord.mX <= dMaxX)
									&& (pixelCoord.mY >= dMinY)
									&& (pixelCoord.mY <= dMaxY))
								{
									LayerList *pLayerList =
										pSpatialDataView->getLayerList();
									if (pLayerList != NULL)
									{
										RasterElement *pRaster =
											pLayerList->
											getPrimaryRasterElement();
										if (pRaster != NULL)
										{
											Layer *pLayer =
												pLayerList->getLayer(RASTER,
																	 pRaster);
											if (pLayer != NULL)
											{
												LocationType dataCoord;
												pLayer->
													translateWorldToData
													(pixelCoord.mX,
													 pixelCoord.mY,
													 dataCoord.mX,
													 dataCoord.mY);
												...
												if (pDescriptor != NULL)
												{
														   ...
														   isValid()))
													{
															...
															NULL)
														{
															...
															if (correct[0] > 255 || correct[1] > 255 || correct[2] > 255)	// if 
																															// image 
																															// is 
																															// 16-bit
															{
															...
															RasterDataDescriptor
																...
																*
																>(dRaster->
																  getDataDescriptor
																  ());
															rDesc->setDisplayMode(RGB_MODE);	// enable 
																								// color 
																								// mode
															rDesc->
																setDisplayBand
						...
						// pSpatialDataView->setMouseMode(NULL); //disable
						// mouse mode
...
void neutralwhite::windowAdded(Subject & subject, const string & signal,
							   const boost::any & value)
{
	if (dynamic_cast < DesktopServices * >(&subject) != NULL)
	{
		...
		SpatialDataWindow *pSpatialDataWindow =
			dynamic_cast < SpatialDataWindow * >(pWindow);
...
void neutralwhite::windowActivated(Subject & subject, const string & signal,
								   const boost::any & value)
{
	enableAction();
}

void neutralwhite::windowRemoved(Subject & subject, const string & signal,
								 const boost::any & value)
{
	if (dynamic_cast < DesktopServices * >(&subject) != NULL)
	{
...
void neutralwhite::addMouseMode(SpatialDataView * pView)
{
	...
	if (mpMouseMode == NULL)
	{
		...
		mpMouseMode =
			pDesktop->createMouseMode("DisplayPixelCoordinateMode2", NULL,
									  NULL, -1, -1, mpMouseModeAction3);
	}
...
void neutralwhite::enableAction()
{
	if (mpMouseModeAction3 != NULL)
	{
		...
		SpatialDataWindow *pWindow =
			dynamic_cast <
			SpatialDataWindow * >(pDesktop->getCurrentWorkspaceWindow());
		if (pWindow != NULL)
		{
			SpatialDataView *pView = pWindow->getSpatialDataView();
			if (pView != NULL)
