...
bool median_noise::copyImage3(RasterElement * pRaster, RasterElement * dRaster, int i, Progress * pProgress, const int size)
{
	...
	for (unsigned int curRow = 0; curRow < pDesc->getRowCount(); ++curRow)
	{
		...
		{
			switchOnEncoding(pDesc->getDataType(), medianfilter, pDestAcc->getColumn(), thirdBandDa, curRow, curCol, pDesc->getRowCount(), pDesc->getColumnCount(), size);
			pDestAcc->nextColumn();
		}
		pDestAcc->nextRow();
	}
...
bool median_noise::execute(PlugInArgList * pInArgList, PlugInArgList * pOutArgList)
{
	...
	if (stat == QDialog::Accepted)
	{
		...
		RasterElement *dRas = RasterUtilities::createRasterElement(pCube->getName() + "Noise_reduction_Median_filter", pDesc->getRowCount(), pDesc->getColumnCount(), 3, pDesc->getDataType(), BSQ);
		...
		if (!isBatch())
		{
			...
			SpatialDataWindow *pWindow =
				static_cast <SpatialDataWindow *>(pDesktop->createWindow(pResultCube->getName(), SPATIAL_DATA_WINDOW));
