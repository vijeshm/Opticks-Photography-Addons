...
{
	template < typename T >
		void nonlocalmeans(T * pData, int row, int col, const int rowSize,
						   const int colSize, double **image,
						   const double SIGMA, const int size, const int csize)
	{
		...
		for (r = size / 2; r >= 0; r--, row2--)
		{
			...
			while (c >= 0)
			{
				...
				qrow2 = row2;
				qcol2 = col2;	// q
				for (pr = csize / 2, qr = csize / 2; pr >= 0 && qr >= 0;
					 pr--, qr--, prow2--, qrow2--)
				{
					// p
					if (pr == (int)csize / 2)
					...
					{
						// p
						// pSrcAcc->toPixel(std::max(prow2,0),
						// std::max(pcol2,0));
						...
						// q
						// pSrcAcc->toPixel(std::max(qrow2,0),
						// std::max(qcol2,0));
						// VERIFYNRV(pSrcAcc.isValid());
				...
				for (pr = csize / 2, qr = csize / 2; pr < csize && qr < csize;
					 pr++, qr++, prow2++, qrow2++)
				{
					// p
					if (pr == (int)csize / 2)
					...
					while (pc < csize && qc < csize)
					{
						...
						// pSrcAcc->toPixel(std::min(rtemp,rowSize-1),
						// std::min(ctemp,colSize-1));
						// VERIFYNRV(pSrcAcc.isValid());
						// p= *reinterpret_cast<T*>(pSrcAcc->getColumn());
						p = image[std::min(rtemp, rowSize - 1)][std::
																min(ctemp,
																	colSize -
																	1)];
						...
						// VERIFYNRV(pSrcAcc.isValid());
						// q= *reinterpret_cast<T*>(pSrcAcc->getColumn());
						q = image[std::min(rtemp, rowSize - 1)][std::
																min(ctemp,
																	colSize -
																	1)];
						// result
			...
			while (c < size)
			{
					 ...
					 pr--, qr--, prow2--, qrow2--)
				{
					...
					while (pc >= 0 && qc >= 0)
					{
						...
						// pSrcAcc->toPixel(std::max(rtemp,0),
						// std::max(ctemp,0));
						// VERIFYNRV(pSrcAcc.isValid());
						// p= *reinterpret_cast<T*>(pSrcAcc->getColumn());
					 ...
					 pr++, qr++, prow2++, qrow2++)
				{
					...
					{
						// p
						// pSrcAcc->toPixel(std::min(prow2,rowSize-1),
						// std::min(pcol2,colSize-1));
						// VERIFYNRV(pSrcAcc.isValid());
						// p= *reinterpret_cast<T*>(pSrcAcc->getColumn());
						p = image[std::min(prow2, rowSize - 1)][std::
																min(pcol2,
																	colSize -
																	1)];

						// q
						// pSrcAcc->toPixel(std::min(qrow2,rowSize-1),
						// std::min(qcol2,colSize-1));
						// VERIFYNRV(pSrcAcc.isValid());
						// q= *reinterpret_cast<T*>(pSrcAcc->getColumn());
						q = image[std::min(qrow2, rowSize - 1)][std::
																min(qcol2,
																	colSize -
																	1)];
				...
				// pSrcAcc->toPixel(std::min(row2,rowSize-1),
				// std::min(col2,colSize-1));
				// VERIFYNRV(pSrcAcc.isValid());
				// data[r][c]= *reinterpret_cast<T*>(pSrcAcc->getColumn());
				data[r][c] =
					image[std::min(row2, rowSize - 1)][std::
													   min(col2, colSize - 1)];
				res = 0;		// reset res
				c++;
				col2++;
		...
		/* 
		   //for central pixel max=weights[0][0];
		   for(count=0;count<size;count++) for(count2=0;count2<size;count2++)
...
bool nlmean::standard_deviation(RasterElement * pRaster,
								RasterElement * dRaster, int i,
								const double SIGMA, const int size,
								const int csize, Progress * pProgress)
{
	VERIFY(pRaster != NULL);
	...
	for (unsigned int indx = 0; indx < rowSize; ++indx)
	...
	for (unsigned int curRow = 0; curRow < pDesc->getRowCount(); ++curRow)
	{
			 ...
			 ++curCol)
		{
			...
			// switchOnEncoding(pDesc->getDataType(), nonlocalmeans,
			// pDestAcc->getColumn(), thirdBandDa, curRow, curCol,
			// pDesc->getRowCount(), pDesc->getColumnCount());

			// msg="Working on "+StringUtilities::toDisplayString(curRow)+" ,
			// "+StringUtilities::toDisplayString(curCol);
	...
	for (unsigned int curRow = 0; curRow < pDesc->getRowCount(); ++curRow)
	{
			 ...
			 ++curCol)
		{
			...
			switchOnEncoding(pDesc->getDataType(), nonlocalmeans,
							 pDestAcc->getColumn(), curRow, curCol,
							 pDesc->getRowCount(), pDesc->getColumnCount(),
							 image, SIGMA, size, csize);
	...
	// deletion
	for (unsigned int indx2 = 0; indx2 < rowSize; ++indx2)
		delete[]image[indx2];
	delete[]image;
...
{
	VERIFY(pInArgList =
		   Service < PlugInManagerServices > ()->getPlugInArgList());
	pInArgList->addArg < Progress > (Executable::ProgressArg(), NULL,
									 "Progress reporter");
	pInArgList->addArg < RasterElement > (Executable::DataElementArg(),
										  "Perform noise reduction on this data element");
	return true;
}
...
{
	StepResource pStep("nlmean", "app",
					   "5EA0CC75-9E0B-4c3d-BA23-6DB7157BBD78");
	...
	pOutArgList->setPlugInArgValue("nlmean", pResultCube.release());	// for 
																		// saving 
																		// data
