...
bool neutralwhite::getRGB(RasterElement *pRaster, RasterDataDescriptor* pDesc, int x, int y, double correct[])
{
  ...
  SpatialDataView* pSpatialDataView = dynamic_cast<SpatialDataView*>(pDesktop->getCurrentWorkspaceWindowView());
  if (pSpatialDataView != NULL)
       {
        pViewWidget2 = pSpatialDataView->getWidget();
       }

   QMessageBox::information(pViewWidget2, "Display RGB values", "RGB values of the selected pixel are (" + QString::number(correct[0]) + ", " + QString::number(correct[1]) + ", " + QString::number(correct[2]) + ")");
...
{   
	...
	DimensionDescriptor thirdBand = pDesc->getActiveBand(i);   //get active band source
   FactoryResource<DataRequest> pRequest;
   pRequest->setInterleaveFormat(BSQ);
   ...
   for (unsigned int curRow = 0; curRow < pDesc->getRowCount(); ++curRow)
   {
	   ...
	   {	  
		...
		switchOnEncoding(pDesc->getDataType(), copywhite2, pDestAcc->getColumn(), thirdBandDa, curRow, curCol, pDesc->getRowCount(), pDesc->getColumnCount(),correct,max);
...
neutralwhite::neutralwhite() :mpMouseMode(NULL), mpMouseModeAction3(NULL)
{
