...
nlmean_ui::nlmean_ui(QWidget * pParent):QDialog(pParent)
{
	...
	connect(SigmaValueBox, SIGNAL(valueChanged(double)), this,
			SLOT(setSigmaValue(double)));
	connect(ResearchBox, SIGNAL(valueChanged(double)), this,
			SLOT(setResearchWindow(double)));
	connect(CompareBox, SIGNAL(valueChanged(double)), this,
			SLOT(setCompareWindow(double)));
