...
{
	template < typename T >
		void adaptivemedian(T * pData, DataAccessor pSrcAcc, int row, int col,
							int rowSize, int colSize, int size, int sizeMax,
							Progress * pProgress, int *flagptr)
	{
		...
		for (r = size / 2; r >= 0; r--, row2--)
		{
			...
			while (c >= 0)
			{
				...
				sampledata[ctr++] =
					*reinterpret_cast < T * >(pSrcAcc->getColumn());
				c--;
				col2--;
			}
		}
