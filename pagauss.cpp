...
{
	template < typename T >
		void gauss(T * pData, DataAccessor pSrcAcc, int row, int col,
				   int rowSize, int colSize)
	{
		...
		/* 
		   double gx = -1.0 * upperLeftVal + -2.0 * leftVal + -1.0 *
		   lowerLeftVal + 1.0 * upperRightVal + 2.0 * rightVal + 1.0 *
		   lowerRightVal; double gy = -1.0 * lowerLeftVal + -2.0 * downVal +
		   -1.0 * lowerRightVal + 1.0 * upperLeftVal + 2.0 * upVal + 1.0 *
		   upperRightVal; double magnitude = sqrt(gx * gx + gy * gy);
		...
		// gaussian
		double gx =
			0.0625 * upperLeftVal + 0.125 * leftVal + 0.0625 * lowerLeftVal +
			0.0625 * upperRightVal + 0.125 * rightVal +
			0.0625 * lowerRightVal + 0.125 * upVal + 0.125 * downVal +
			0.25 * midVal;
...
bool pagauss::getInputSpecification(PlugInArgList * &pInArgList)
{
	...
	pInArgList->addArg < RasterElement > (Executable::DataElementArg(),
										  "Perform edge detection on this data element");
	return true;
}
...
{
	StepResource pStep("Tutorial 5", "app",
	...
	{
		std::string msg =
			"Edge detection cannot be performed on complex types.";
		pStep->finalize(Message::Failure, msg);
		if (pProgress != NULL)
		{
			pProgress->updateProgress(msg, 0, ERRORS);
	...
	ModelResource < RasterElement >
		pResultCube(RasterUtilities::
					createRasterElement(pCube->getName() +
	...
	DataAccessor pDestAcc =
		pResultCube->getDataAccessor(pResultRequest.release());
	...
	for (long signed int row = 0; row < pDesc->getRowCount(); ++row)
	{
		if (pProgress != NULL)
		{
			pProgress->updateProgress("Calculating result",
									  row * 100 / pDesc->getRowCount(),
									  NORMAL);
		}
		if (isAborted())
		{
		...
		{
			switchOnEncoding(pDesc->getDataType(), gauss,
							 pDestAcc->getColumn(), pSrcAcc, row, col,
