...
namespace
{
	double getValChannel(RasterElement * pRaster, int row, int col, int i)
	{
		...
		RasterDataDescriptor *pDesc =
			dynamic_cast <
			RasterDataDescriptor * >(pRaster->getDataDescriptor());
	...
	template < typename T >
		void bilinear(T * pData, DataAccessor pSrcAcc, int row, int col,
					  int rowSize, int colSize, int i, RasterElement * pRaster)
	{
		...
		if (max == rgb[0])		// RED pixel
		{
			...
			{
				// green = (green[x][y-1] + green[x-1][y] + green[x+1][y] +
				// green[x][y+1]) / 4;
				temp = 0;
				// green[x][y-1]
			...
			{
				// blue = (blue[x-1][y-1] + blue[x+1][y-1] + blue[x-1][y+1] +
				// blue[x+1][y+1]) / 4;
				temp = 0;
				// blue[x-1][y-1]
				...
				// blue[x+1][y-1]
				pSrcAcc->toPixel(std::max(row - 1, 0),
								 std::min(col + 1, colSize - 1));
				VERIFYNRV(pSrcAcc.isValid());
				temp += *reinterpret_cast < T * >(pSrcAcc->getColumn());

				// blue[x-1][y+1]
				pSrcAcc->toPixel(std::min(row + 1, rowSize - 1),
								 std::max(col - 1, 0));
				VERIFYNRV(pSrcAcc.isValid());
				temp += *reinterpret_cast < T * >(pSrcAcc->getColumn());
		...
		else if (max == rgb[2])	// BLUE pixel
		{
			...
			{					// green = (green[x][y-1] + green[x-1][y] +
								// green[x+1][y] + green[x][y+1]) / 4;
				temp = 0;
				// green[x][y-1]
				pSrcAcc->toPixel(std::max(row - 1, 0), col);
				VERIFYNRV(pSrcAcc.isValid());
			...
			{					// red = (red[x-1][y-1] + red[x+1][y-1] +
								// red[x-1][y+1] + red[x+1][y+1]) / 4;
				temp = 0;
				// red[x-1][y-1]
				pSrcAcc->toPixel(std::max(row - 1, 0), std::max(col - 1, 0));
				VERIFYNRV(pSrcAcc.isValid());
